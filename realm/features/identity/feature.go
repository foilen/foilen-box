...
func (f *Feature) Push(to, name string, kp model.KeyPair) (err error) {
	defer func() {
		if err != nil {
			log.Printf("realm identity: push of identity %q to %s failed: %v", name, f.peerLabel(to), err)
		} else {
			log.Printf("realm identity: pushed identity %q to %s", name, f.peerLabel(to))
		}
	}()
...
func (f *Feature) handlePushStream(reg *realm.Registrar) network.StreamHandler {
	return func(s network.Stream) {
		...
		if err := json.NewDecoder(io.LimitReader(s, maxBytes)).Decode(&req); err != nil {
			log.Printf("realm identity: failed to decode pushed identity from %s: %v", f.peerLabel(remote.String()), err)
			return
		}

		if !reg.IsAllowed(remote, ActionPush) {
			log.Printf("realm identity: pushed identity from %s rejected: no permission", f.peerLabel(remote.String()))
		...
		if err != nil {
			log.Printf("realm identity: pushed identity from %s has an invalid key: %v", f.peerLabel(remote.String()), err)
		...
		if err := onReceive(req.Name, kp); err != nil {
			log.Printf("realm identity: failed to import identity %q pushed from %s: %v", req.Name, f.peerLabel(remote.String()), err)
			_ = json.NewEncoder(s).Encode(pushAck{Imported: false, Error: err.Error()})
			return
		}

		log.Printf("realm identity: imported identity %q pushed from %s", req.Name, f.peerLabel(remote.String()))
